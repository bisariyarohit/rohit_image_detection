...
class PoseAttention(nn.Module):
    def __init__(self, feat_dim, pose_dim):
        super().__init__()
        self.fc_pose = nn.Linear(pose_dim, feat_dim)
        self.sigmoid = nn.Sigmoid()

    def forward(self, feat, pose_sig):
        # pose_sig: [batch, pose_dim]
        attn = self.sigmoid(self.fc_pose(pose_sig))  # [batch, feat_dim]
        return feat * attn  # Element-wise modulation

# ...existing code...
# from model.pose_attention import PoseAttention

class MainModel(nn.Module):
    def __init__(self):
        # ...existing code...
        self.pose_attention = PoseAttention(feat_dim=768, pose_dim=16)  # Adjust dims as needed

    def forward(self, x, pose_sig=None, inference=False, **kwargs):
        # ...existing code...
        feat = self.final_feat_layer(x)
        if inference and pose_sig is not None:
            feat = self.pose_attention(feat, pose_sig)
        # ...existing code...
        return feat
def do_train(cfg,
             model,
    ...
    if device:
        model.to(local_rank)
        pose_model.to(local_rank)
        arc_criterion.to(local_rank)
        cls_loss.to(local_rank)
        # fusion = Fusion(768).to(local_rank)
        if torch.cuda.device_count() > 1 and cfg.MODEL.DIST_TRAIN:
            logger.info('Using {} GPUs for training'.format(torch.cuda.device_count()))
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[local_rank], find_unused_parameters=True)
            pose_model = torch.nn.parallel.DistributedDataParallel(pose_model, device_ids=[local_rank], find_unused_parameters=True)
    ...
    # train
    pose_opt = torch.optim.SGD(pose_model.parameters(), lr=0.001, momentum=0.9)
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader):
            ...
            # if isinstance(pose_score, list):
            #     print(pose_score.max(1)[1], target)
            #     pose_acc = (pose_score[0].max(1)[1] == target).float().mean()
            # else:
            #     # print(pose_score)
            #     print(torch.max(pose_score, 1), target)
            #     pose_acc = (torch.max(pose_score, 1) == target).sum()/64
            ...
            if cfg.MODEL.DIST_TRAIN:
                if dist.get_rank() == 0:
                    if (n_iter + 1) % log_period == 0:
                        ...
                        logger.info("Epoch[{}] Iter[{}/{}] Loss: {:.3f}, Loss_pose: {:.3f}, Acc: {:.3f}, Base Lr: {:.2e}"
